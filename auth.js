...
(() => {
    ...
    function bindEvents() {
        ...
        // Delegate the two primary entry actions. This survives mobile
        // back/forward-cache restores and avoids stale element listeners.
    ...
    function promptForSignIn(message) {
        hideAuthMessage();
        hideDashboardMessage();
        App.showScreen("profile");
        ...
        if (!account.configured) {
            showSignedOutState();
            showAuthMessage(
                "The account service is not connected yet. Please try again later.",
                "error"
            );
            return;
        }
        ...
        if (account.user) {
            showDashboardState();
            showDashboardMessage(message, "info");
            return;
        }
    ...
    async function startNewReading() {
        ...
        try {
            ...
            // Save locally and open the quiz before waiting for the network.
            // This prevents the button from appearing unresponsive on phones.
            App.saveProgress();
            if (!beginOrResumeQuiz()) {
                throw new Error("The quiz did not finish loading. Refresh the page and try again.");
            }

            await ensureActiveReading();
            App.saveProgress();
        } catch (error) {
            console.error("A new account reading could not be prepared.", error);
            if (App.elements.savedStatus) {
                App.elements.savedStatus.textContent =
                    "The reading is saved on this device, but account syncing is temporarily unavailable.";
            }
        } finally {
