...
(() => {
    ...
    const PENDING_ACTION_KEY = "marriageOraclePendingUnlockActionV1";
    const PENDING_PROMO_CODE_KEY = "marriageOraclePendingPromoCodeV1";

    function purchaseConsentCheckbox() {
        return document.getElementById("purchase-consent-checkbox");
    }

    function checkoutMessageElement() {
    ...
    async function handleUnlock() {
        ...
        if (!consent) {
            showCheckoutMessage(
                "This page is missing the purchase acknowledgement. Refresh the website and try again.",
                "error"
            );
            return;
        }
        ...
        if (!consent.checked) {
            ...
            consent.focus();
            consent.closest(".purchase-consent-row")?.scrollIntoView({
        ...
        if (!auth.user) {
            rememberPendingAction("checkout");
            auth.promptForSignIn?.(
                "Sign in or create an account to pay for this reading. Your completed reading has been kept."
            );
            return;
        }
        ...
        try {
            ...
            checkoutMessageElement()?.scrollIntoView({
                behavior: App.utils.prefersReducedMotion() ? "auto" : "smooth",
                block: "center"
            });
        } finally {
    ...
    async function redeemPromoCode(event) {
        ...
        if (!auth.user) {
            rememberPendingAction("promo", code);
            auth.promptForSignIn?.(
                "Sign in or create an account to use a complimentary code. Your completed reading and entered code have been kept."
            );
            return;
        }
    ...
    async function continuePendingUnlockAfterSignIn() {
        ...
        const action = App.storage.readText(
            "sessionStorage",
            PENDING_ACTION_KEY,
            "checkout"
        );
        const pendingPromoCode = App.storage.readText(
            "sessionStorage",
            PENDING_PROMO_CODE_KEY,
            ""
        );
        ...
        if (action === "promo") {
            if (App.elements.promoInput && pendingPromoCode) {
                App.elements.promoInput.value = pendingPromoCode;
            }
            showPromoMessage(
                "You are signed in. Press Apply Code to use your complimentary code.",
                "success"
            );
            App.elements.promoInput?.scrollIntoView({
                behavior: App.utils.prefersReducedMotion() ? "auto" : "smooth",
                block: "center"
            });
            return;
        ...
        showCheckoutMessage(
            "You are signed in. Tick the purchase acknowledgement, then press Pay £0.99 & Unlock Reading.",
            "success"
        );
        purchaseConsentCheckbox()?.scrollIntoView({
            behavior: App.utils.prefersReducedMotion() ? "auto" : "smooth",
            block: "center"
        });
    }
    ...
    function rememberPendingAction(action, promoCode = "") {
        App.storage.writeText(
            "sessionStorage",
            App.storageKeys.pendingUnlock,
            "true"
        );
        App.storage.writeText("sessionStorage", PENDING_ACTION_KEY, action);
    ...
    function clearPendingAction() {
        ...
        App.storage.removeStoredValue("sessionStorage", PENDING_ACTION_KEY);
        App.storage.removeStoredValue("sessionStorage", PENDING_PROMO_CODE_KEY);
    }

    function showCheckoutMessage(message, type = "info") {
        const element = checkoutMessageElement();
        if (!element) {
    ...
    function syncPurchaseConsent() {
        ...
        const consent = purchaseConsentCheckbox();
        if (!button || !consent || button.dataset.originalText) return;

        button.disabled = false;
        button.removeAttribute("aria-disabled");
        button.dataset.consentReady = String(consent.checked);
        button.classList.toggle("needs-consent", !consent.checked);
